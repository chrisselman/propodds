...
setwd("/group/cebu1/BACKUP/Chris/Project_2/Main_Simulations/Moderate_n/Skewed/Seven_categories/Random_PO/Moderate_effect/Moderate")
...
load("smalln_skew_7cat_randompo_modeff_moderate_dataset.Rdata")
attach(big_data)
...
for (k in s1[datnum]:s2[datnum]){
  ...
  bias_cumlogit_po1[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - log(1.50)
  bias_cumlogit_po2[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - log(1.50)
  bias_cumlogit_po3[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - log(1.50)
  bias_cumlogit_po4[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - log(1.50)
  bias_cumlogit_po5[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - log(1.50)
  ...
  # MSE 
  mse_cumlogit_po1[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - log(1.50))^2
  mse_cumlogit_po2[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - log(1.50))^2
  mse_cumlogit_po3[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - log(1.50))^2
  mse_cumlogit_po4[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - log(1.50))^2
  mse_cumlogit_po5[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - log(1.50))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_ppo1[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - log(1.50)
  ...
  # MSE 
  mse_cumlogit_ppo1[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - log(1.50))^2
  ...
  cppomoddiv <- blrm(y~x,ppo = ~x, normcppo=F,
                     priorsdppo = 100,
                     conc = 1, seed=1234,iter=7500,chains=4,cppo=function(y) y==7,sampling.args = list(cores = 4,control=list(adapt_delta=0.85,max_treedepth=12)))
...
save(data, file = paste0("smalln_skew_7cat_randompo_modeff_moderate",datnum,".Rdata"))
