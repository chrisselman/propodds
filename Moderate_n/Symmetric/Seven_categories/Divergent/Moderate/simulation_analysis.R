...
setwd("/group/cebu1/BACKUP/Chris/Project_2/Main_Simulations/Moderate_n/Symmetric/Seven_categories/Divergent/Moderate")
...
load("smalln_sym_7cat_diverg_moderate_dataset.Rdata")
attach(big_data)
...
for (k in s1[datnum]:s2[datnum]){
  ...
  bias_cumlogit_po1[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - log(1)
  bias_cumlogit_po2[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - log(1)
  bias_cumlogit_po3[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - log(1)
  bias_cumlogit_po4[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - log(1)
  bias_cumlogit_po5[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - log(1)
  bias_cumlogit_po6[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - log(1.50)
  ...
  # MSE 
  mse_cumlogit_po1[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - log(1))^2
  mse_cumlogit_po2[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - log(1))^2
  mse_cumlogit_po3[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - log(1))^2
  mse_cumlogit_po4[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - log(1))^2
  mse_cumlogit_po5[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - log(1))^2
  mse_cumlogit_po6[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - log(1.50))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_ppo1[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - log(1)
  ...
  # MSE 
  mse_cumlogit_ppo1[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - log(1))^2
...
save(data, file = paste0("smalln_sym_7cat_diverg_moderate",datnum,".Rdata"))
