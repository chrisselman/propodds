...
setwd("/group/cebu1/BACKUP/Chris/Project_2/Main_Simulations/Moderate_n/Symmetric/Seven_categories/Linear")
...
load("smalln_sym_7cat_linear_dataset.Rdata")
attach(big_data)
...
for (k in s1[datnum]:s2[datnum]){
  ...
  bias_cumlogit_po1[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - (log(0.8) + 0.06*0)
  bias_cumlogit_po2[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - (log(0.8) + 0.06*1)
  bias_cumlogit_po3[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - (log(0.8) + 0.06*2)
  bias_cumlogit_po4[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - (log(0.8) + 0.06*3)
  bias_cumlogit_po5[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - (log(0.8) + 0.06*4)
  bias_cumlogit_po6[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - (log(0.8) + 0.06*5)
  ...
  # MSE 
  mse_cumlogit_po1[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - (log(0.8) + 0.06*0))^2
  mse_cumlogit_po2[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - (log(0.8) + 0.06*1))^2
  mse_cumlogit_po3[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - (log(0.8) + 0.06*2))^2
  mse_cumlogit_po4[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - (log(0.8) + 0.06*3))^2
  mse_cumlogit_po5[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - (log(0.8) + 0.06*4))^2
  mse_cumlogit_po6[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - (log(0.8) + 0.06*5))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_ppo1[k] <- as.numeric(summarise_draws(df, "median")[7,2]) - (log(0.8) + 0.06*0)
  ...
  # MSE 
  mse_cumlogit_ppo1[k] <- (as.numeric(summarise_draws(df, "median")[7,2]) - (log(0.8) + 0.06*0))^2
  ...
  cppomoddiv <- blrm(y~x,ppo = ~x, normcppo=F,
                     priorsdppo = 100,
                     conc = 1, seed=1234,iter=7500,chains=4,cppo=function(y) y==7,sampling.args = list(cores = 4,control=list(adapt_delta=0.9,max_treedepth=12)))
...
save(data, file = paste0("smalln_sym_7cat_linear",datnum,".Rdata"))
