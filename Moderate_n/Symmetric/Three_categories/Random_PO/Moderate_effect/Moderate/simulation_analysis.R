...
setwd("/group/cebu1/BACKUP/Chris/Project_2/Main_Simulations/Moderate_n/Symmetric/Three_categories/Random_PO/Moderate_effect/Moderate")
...
load("smalln_sym_3cat_randompo_modeff_moderate_dataset.Rdata")
attach(big_data)
...
for (k in s1[datnum]:s2[datnum]){
  ...
  bias_cumlogit_po1[k] <- as.numeric(summarise_draws(df, "median")[3,2]) - log(1.50)
  ...
  # MSE 
  mse_cumlogit_po1[k] <- (as.numeric(summarise_draws(df, "median")[3,2]) - log(1.50))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_ppo1[k] <- as.numeric(summarise_draws(df, "median")[3,2]) - log(1.50)
  ...
  # MSE 
  mse_cumlogit_ppo1[k] <- (as.numeric(summarise_draws(df, "median")[3,2]) - log(1.50))^2
...
save(data, file = paste0("smalln_sym_3cat_randompo_modeff_moderate",datnum,".Rdata"))
