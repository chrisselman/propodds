...
setwd("/group/cebu1/BACKUP/Chris/Project_2/Main_Simulations/Moderate_n/Symmetric/Three_categories/Divergent/Small_effect")
...
load("smalln_sym_3cat_diverg_smalleff_dataset.Rdata")
attach(big_data)
...
for (k in s1[datnum]:s2[datnum]){
  ...
  bias_cumlogit_po2[k] <- as.numeric(summarise_draws(df, "median")[3,2]) - log(1.10)
  ...
  mse_cumlogit_po2[k] <- (as.numeric(summarise_draws(df, "median")[3,2]) - log(1.10))^2
  ...
  ################### LOGISTIC MODEL - FIRST CUMULATIVE LOGIT (cat 3 vs 2 and 1) ################
  logmod2 <- blrm(y2~x1, 
                  pcontrast=list(c1=.(x1='0'), c2=.(x1='1'),
                                 contrast=expression(c2-c1), sd=100),
                  conc = 1, seed=1234,iter=7500,chains=4,sampling.args = list(cores = 4))
...
save(data, file = paste0("smalln_sym_3cat_diverg_smalleff",datnum,".Rdata"))
