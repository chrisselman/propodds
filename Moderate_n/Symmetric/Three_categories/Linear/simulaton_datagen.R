...
setwd("/group/cebu1/BACKUP/Chris/Project_2/Main_Simulations/Moderate_n/Symmetric/Three_categories/Linear")
...
quant <- pbeta(c(1/length(states) * seq(1,length(states)-1,1)),1.8,1.8)
...
## Define functions
getProbs <- function(p0 = c(10, 40, 10,40)/100, logOddsRatio)
{
  cumProbs0   <- cumsum(p0)
  ...
  for (i in 1:(length(p0)-1)){
    cumlogOdds1[i] <- cumlogOdds0[i] + logOddsRatio - (i-1)*0.06
  }
...
save(big_data, file = "smalln_sym_3cat_linear_dataset.Rdata")
