...
setwd("/group/cebu1/BACKUP/Chris/Project_2/Main_Simulations/Moderate_n/Symmetric/Eleven_categories/Random_PO/Moderate_effect/None")
...
load("smalln_sym_11cat_randompo_modeff_none_dataset.Rdata")
attach(big_data)
...
for (k in s1[datnum]:s2[datnum]){
  ...
  ppomod <- blrm(y~x,ppo = ~x, priorsdppo = 100,
                 conc = 1, seed=1234,iter=7500,chains=4,sampling.args = list(cores = 4,control=list(adapt_delta=0.999,max_treedepth=30)))
...
save(data, file = paste0("smalln_sym_11cat_randompo_modeff_none",datnum,".Rdata"))
