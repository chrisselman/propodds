...
setwd("/group/cebu1/BACKUP/Chris/Project_2/Main_Simulations/Moderate_n/Symmetric/Eleven_categories/Divergent/Slight")
...
load("smalln_sym_11cat_diverg_slight_dataset.Rdata")
attach(big_data)
...
for (k in s1[datnum]:s2[datnum]){
  ...
  bias_cumlogit_po1[k] <- as.numeric(summarise_draws(df, "median")[11,2]) - log(1)
  bias_cumlogit_po2[k] <- as.numeric(summarise_draws(df, "median")[11,2]) - log(1)
  bias_cumlogit_po3[k] <- as.numeric(summarise_draws(df, "median")[11,2]) - log(1)
  bias_cumlogit_po4[k] <- as.numeric(summarise_draws(df, "median")[11,2]) - log(1)
  bias_cumlogit_po5[k] <- as.numeric(summarise_draws(df, "median")[11,2]) - log(1)
  bias_cumlogit_po6[k] <- as.numeric(summarise_draws(df, "median")[11,2]) - log(1)
  bias_cumlogit_po7[k] <- as.numeric(summarise_draws(df, "median")[11,2]) - log(1)
  bias_cumlogit_po8[k] <- as.numeric(summarise_draws(df, "median")[11,2]) - log(1)
  bias_cumlogit_po9[k] <- as.numeric(summarise_draws(df, "median")[11,2]) - log(1)
  bias_cumlogit_po10[k] <- as.numeric(summarise_draws(df, "median")[11,2]) - log(1.10)
  ...
  cv_cumlogit_po1[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  cv_cumlogit_po2[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  cv_cumlogit_po3[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  cv_cumlogit_po4[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  cv_cumlogit_po5[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  cv_cumlogit_po6[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  cv_cumlogit_po7[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  cv_cumlogit_po8[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  cv_cumlogit_po9[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  cv_cumlogit_po10[k] <- ifelse(lci < log(1.10) & uci > log(1.10), 1, 0)

  
  # MSE 
  mse_cumlogit_po1[k] <- (as.numeric(summarise_draws(df, "median")[11,2]) - log(1))^2
  mse_cumlogit_po2[k] <- (as.numeric(summarise_draws(df, "median")[11,2]) - log(1))^2
  mse_cumlogit_po3[k] <- (as.numeric(summarise_draws(df, "median")[11,2]) - log(1))^2
  mse_cumlogit_po4[k] <- (as.numeric(summarise_draws(df, "median")[11,2]) - log(1))^2
  mse_cumlogit_po5[k] <- (as.numeric(summarise_draws(df, "median")[11,2]) - log(1))^2
  mse_cumlogit_po6[k] <- (as.numeric(summarise_draws(df, "median")[11,2]) - log(1))^2
  mse_cumlogit_po7[k] <- (as.numeric(summarise_draws(df, "median")[11,2]) - log(1))^2
  mse_cumlogit_po8[k] <- (as.numeric(summarise_draws(df, "median")[11,2]) - log(1))^2
  mse_cumlogit_po9[k] <- (as.numeric(summarise_draws(df, "median")[11,2]) - log(1))^2
  mse_cumlogit_po10[k] <- (as.numeric(summarise_draws(df, "median")[11,2]) - log(1.10))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_ppo1[k] <- as.numeric(summarise_draws(df, "median")[11,2]) - log(1)
  bias_cumlogit_ppo2[k] <- as.numeric(summarise_draws(phi1, "median")[1,2]) - log(1)
  bias_cumlogit_ppo3[k] <- as.numeric(summarise_draws(phi2, "median")[1,2]) - log(1)
  bias_cumlogit_ppo4[k] <- as.numeric(summarise_draws(phi3, "median")[1,2]) - log(1)
  bias_cumlogit_ppo5[k] <- as.numeric(summarise_draws(phi4, "median")[1,2]) - log(1)
  bias_cumlogit_ppo6[k] <- as.numeric(summarise_draws(phi5, "median")[1,2]) - log(1)
  bias_cumlogit_ppo7[k] <- as.numeric(summarise_draws(phi6, "median")[1,2]) - log(1)
  bias_cumlogit_ppo8[k] <- as.numeric(summarise_draws(phi7, "median")[1,2]) - log(1)
  bias_cumlogit_ppo9[k] <- as.numeric(summarise_draws(phi8, "median")[1,2]) - log(1)
  bias_cumlogit_ppo10[k] <- as.numeric(summarise_draws(phi9, "median")[1,2]) - log(1.10)
  ...
  cv_cumlogit_ppo1[k] <- ifelse(lci1 < log(1) & uci1 > log(1), 1, 0)
  cv_cumlogit_ppo2[k] <- ifelse(lci2 < log(1) & uci2 > log(1), 1, 0)
  cv_cumlogit_ppo3[k] <- ifelse(lci3 < log(1) & uci3 > log(1), 1, 0)
  cv_cumlogit_ppo4[k] <- ifelse(lci4 < log(1) & uci4 > log(1), 1, 0)
  cv_cumlogit_ppo5[k] <- ifelse(lci5 < log(1) & uci5 > log(1), 1, 0)
  cv_cumlogit_ppo6[k] <- ifelse(lci6 < log(1) & uci6 > log(1), 1, 0)
  cv_cumlogit_ppo7[k] <- ifelse(lci7 < log(1) & uci7 > log(1), 1, 0)
  cv_cumlogit_ppo8[k] <- ifelse(lci8 < log(1) & uci8 > log(1), 1, 0)
  cv_cumlogit_ppo9[k] <- ifelse(lci9 < log(1) & uci9 > log(1), 1, 0)
  cv_cumlogit_ppo10[k] <- ifelse(lci10 < log(1.10) & uci10 > log(1.10), 1, 0)

  
  # MSE 
  mse_cumlogit_ppo1[k] <- (as.numeric(summarise_draws(df, "median")[11,2]) - log(1))^2
  mse_cumlogit_ppo2[k] <- (as.numeric(summarise_draws(phi1, "median")[1,2]) - log(1))^2
  mse_cumlogit_ppo3[k] <- (as.numeric(summarise_draws(phi2, "median")[1,2]) - log(1))^2
  mse_cumlogit_ppo4[k] <- (as.numeric(summarise_draws(phi3, "median")[1,2]) - log(1))^2
  mse_cumlogit_ppo5[k] <- (as.numeric(summarise_draws(phi4, "median")[1,2]) - log(1))^2
  mse_cumlogit_ppo6[k] <- (as.numeric(summarise_draws(phi5, "median")[1,2]) - log(1))^2
  mse_cumlogit_ppo7[k] <- (as.numeric(summarise_draws(phi6, "median")[1,2]) - log(1))^2
  mse_cumlogit_ppo8[k] <- (as.numeric(summarise_draws(phi7, "median")[1,2]) - log(1))^2
  mse_cumlogit_ppo9[k] <- (as.numeric(summarise_draws(phi8, "median")[1,2]) - log(1))^2
  mse_cumlogit_ppo10[k] <- (as.numeric(summarise_draws(phi9, "median")[1,2]) - log(1.10))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_lcpo1[k] <- as.numeric(summarise_draws(phi1, "median")[1,2]) - log(1)
  bias_cumlogit_lcpo2[k] <- as.numeric(summarise_draws(phi2, "median")[1,2]) - log(1)
  bias_cumlogit_lcpo3[k] <- as.numeric(summarise_draws(phi3, "median")[1,2]) - log(1)
  bias_cumlogit_lcpo4[k] <- as.numeric(summarise_draws(phi4, "median")[1,2]) - log(1)
  bias_cumlogit_lcpo5[k] <- as.numeric(summarise_draws(phi5, "median")[1,2]) - log(1)
  bias_cumlogit_lcpo6[k] <- as.numeric(summarise_draws(phi6, "median")[1,2]) - log(1)
  bias_cumlogit_lcpo7[k] <- as.numeric(summarise_draws(phi7, "median")[1,2]) - log(1)
  bias_cumlogit_lcpo8[k] <- as.numeric(summarise_draws(phi8, "median")[1,2]) - log(1)
  bias_cumlogit_lcpo9[k] <- as.numeric(summarise_draws(phi9, "median")[1,2]) - log(1)
  bias_cumlogit_lcpo10[k] <- as.numeric(summarise_draws(phi10, "median")[1,2]) - log(1.10)
  ...
  cv_cumlogit_lcpo1[k] <- ifelse(lci1 < log(1) & uci1 > log(1), 1, 0)
  cv_cumlogit_lcpo2[k] <- ifelse(lci2 < log(1) & uci2 > log(1), 1, 0)
  cv_cumlogit_lcpo3[k] <- ifelse(lci3 < log(1) & uci3 > log(1), 1, 0)
  cv_cumlogit_lcpo4[k] <- ifelse(lci4 < log(1) & uci4 > log(1), 1, 0)
  cv_cumlogit_lcpo5[k] <- ifelse(lci5 < log(1) & uci5 > log(1), 1, 0)
  cv_cumlogit_lcpo6[k] <- ifelse(lci6 < log(1) & uci6 > log(1), 1, 0)
  cv_cumlogit_lcpo7[k] <- ifelse(lci7 < log(1) & uci7 > log(1), 1, 0)
  cv_cumlogit_lcpo8[k] <- ifelse(lci8 < log(1) & uci8 > log(1), 1, 0)
  cv_cumlogit_lcpo9[k] <- ifelse(lci9 < log(1) & uci9 > log(1), 1, 0)
  cv_cumlogit_lcpo10[k] <- ifelse(lci10 < log(1.10) & uci10 > log(1.10), 1, 0)
  
  # MSE 
  mse_cumlogit_lcpo1[k] <- (as.numeric(summarise_draws(phi1, "median")[1,2]) - log(1))^2
  mse_cumlogit_lcpo2[k] <- (as.numeric(summarise_draws(phi2, "median")[1,2]) - log(1))^2
  mse_cumlogit_lcpo3[k] <- (as.numeric(summarise_draws(phi3, "median")[1,2]) - log(1))^2
  mse_cumlogit_lcpo4[k] <- (as.numeric(summarise_draws(phi4, "median")[1,2]) - log(1))^2
  mse_cumlogit_lcpo5[k] <- (as.numeric(summarise_draws(phi5, "median")[1,2]) - log(1))^2
  mse_cumlogit_lcpo6[k] <- (as.numeric(summarise_draws(phi6, "median")[1,2]) - log(1))^2
  mse_cumlogit_lcpo7[k] <- (as.numeric(summarise_draws(phi7, "median")[1,2]) - log(1))^2
  mse_cumlogit_lcpo8[k] <- (as.numeric(summarise_draws(phi8, "median")[1,2]) - log(1))^2
  mse_cumlogit_lcpo9[k] <- (as.numeric(summarise_draws(phi9, "median")[1,2]) - log(1))^2
  mse_cumlogit_lcpo10[k] <- (as.numeric(summarise_draws(phi10, "median")[1,2]) - log(1.10))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_cpo1[k] <- as.numeric(summarise_draws(phi1, "median")[1,2]) - log(1)
  bias_cumlogit_cpo2[k] <- as.numeric(summarise_draws(phi2, "median")[1,2]) - log(1)
  bias_cumlogit_cpo3[k] <- as.numeric(summarise_draws(phi3, "median")[1,2]) - log(1)
  bias_cumlogit_cpo4[k] <- as.numeric(summarise_draws(phi4, "median")[1,2]) - log(1)
  bias_cumlogit_cpo5[k] <- as.numeric(summarise_draws(phi5, "median")[1,2]) - log(1)
  bias_cumlogit_cpo6[k] <- as.numeric(summarise_draws(phi6, "median")[1,2]) - log(1)
  bias_cumlogit_cpo7[k] <- as.numeric(summarise_draws(phi7, "median")[1,2]) - log(1)
  bias_cumlogit_cpo8[k] <- as.numeric(summarise_draws(phi8, "median")[1,2]) - log(1)
  bias_cumlogit_cpo9[k] <- as.numeric(summarise_draws(phi9, "median")[1,2]) - log(1)
  bias_cumlogit_cpo10[k] <- as.numeric(summarise_draws(phi10, "median")[1,2]) - log(1.10)
  ...
  cv_cumlogit_cpo1[k] <- ifelse(lci1 < log(1) & uci1 > log(1), 1, 0)
  cv_cumlogit_cpo2[k] <- ifelse(lci2 < log(1) & uci2 > log(1), 1, 0)
  cv_cumlogit_cpo3[k] <- ifelse(lci3 < log(1) & uci3 > log(1), 1, 0)
  cv_cumlogit_cpo4[k] <- ifelse(lci4 < log(1) & uci4 > log(1), 1, 0)
  cv_cumlogit_cpo5[k] <- ifelse(lci5 < log(1) & uci5 > log(1), 1, 0)
  cv_cumlogit_cpo6[k] <- ifelse(lci6 < log(1) & uci6 > log(1), 1, 0)
  cv_cumlogit_cpo7[k] <- ifelse(lci7 < log(1) & uci7 > log(1), 1, 0)
  cv_cumlogit_cpo8[k] <- ifelse(lci8 < log(1) & uci8 > log(1), 1, 0)
  cv_cumlogit_cpo9[k] <- ifelse(lci9 < log(1) & uci9 > log(1), 1, 0)
  cv_cumlogit_cpo10[k] <- ifelse(lci10 < log(1.10) & uci10 > log(1.10), 1, 0)

  
  # MSE 
  mse_cumlogit_cpo1[k] <- (as.numeric(summarise_draws(phi1, "median")[1,2]) - log(1))^2
  mse_cumlogit_cpo2[k] <- (as.numeric(summarise_draws(phi2, "median")[1,2]) - log(1))^2
  mse_cumlogit_cpo3[k] <- (as.numeric(summarise_draws(phi3, "median")[1,2]) - log(1))^2
  mse_cumlogit_cpo4[k] <- (as.numeric(summarise_draws(phi4, "median")[1,2]) - log(1))^2
  mse_cumlogit_cpo5[k] <- (as.numeric(summarise_draws(phi5, "median")[1,2]) - log(1))^2
  mse_cumlogit_cpo6[k] <- (as.numeric(summarise_draws(phi6, "median")[1,2]) - log(1))^2
  mse_cumlogit_cpo7[k] <- (as.numeric(summarise_draws(phi7, "median")[1,2]) - log(1))^2
  mse_cumlogit_cpo8[k] <- (as.numeric(summarise_draws(phi8, "median")[1,2]) - log(1))^2
  mse_cumlogit_cpo9[k] <- (as.numeric(summarise_draws(phi9, "median")[1,2]) - log(1))^2
  mse_cumlogit_cpo10[k] <- (as.numeric(summarise_draws(phi10, "median")[1,2]) - log(1.10))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_lr1[k] <- as.numeric(summarise_draws(df, "median")[2,2]) - log(1)
  ...
  cv_cumlogit_lr1[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  
  # MSE 
  mse_cumlogit_lr1[k] <- (as.numeric(summarise_draws(df, "median")[2,2]) - log(1))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_lr2[k] <- as.numeric(summarise_draws(df, "median")[2,2]) - log(1)
  
  # Coverage - does the true value fall in the credible interval?
  lci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,2])
  uci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,3])
  
  cv_cumlogit_lr2[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  
  # MSE 
  mse_cumlogit_lr2[k] <- (as.numeric(summarise_draws(df, "median")[2,2]) - log(1))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_lr3[k] <- as.numeric(summarise_draws(df, "median")[2,2]) - log(1)
  
  # Coverage - does the true value fall in the credible interval?
  lci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,2])
  uci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,3])
  
  cv_cumlogit_lr3[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  
  # MSE 
  mse_cumlogit_lr3[k] <- (as.numeric(summarise_draws(df, "median")[2,2]) - log(1))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_lr4[k] <- as.numeric(summarise_draws(df, "median")[2,2]) - log(1)
  
  # Coverage - does the true value fall in the credible interval?
  lci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,2])
  uci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,3])
  
  cv_cumlogit_lr4[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  
  # MSE 
  mse_cumlogit_lr4[k] <- (as.numeric(summarise_draws(df, "median")[2,2]) - log(1))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_lr5[k] <- as.numeric(summarise_draws(df, "median")[2,2]) - log(1)
  
  # Coverage - does the true value fall in the credible interval?
  lci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,2])
  uci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,3])
  
  cv_cumlogit_lr5[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  
  # MSE 
  mse_cumlogit_lr5[k] <- (as.numeric(summarise_draws(df, "median")[2,2]) - log(1))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_lr6[k] <- as.numeric(summarise_draws(df, "median")[2,2]) - log(1)
  
  # Coverage - does the true value fall in the credible interval?
  lci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,2])
  uci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,3])
  
  cv_cumlogit_lr6[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  
  # MSE 
  mse_cumlogit_lr6[k] <- (as.numeric(summarise_draws(df, "median")[2,2]) - log(1))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_lr7[k] <- as.numeric(summarise_draws(df, "median")[2,2]) - log(1)
  
  # Coverage - does the true value fall in the credible interval?
  lci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,2])
  uci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,3])
  
  cv_cumlogit_lr7[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  
  # MSE 
  mse_cumlogit_lr7[k] <- (as.numeric(summarise_draws(df, "median")[2,2]) - log(1))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_lr8[k] <- as.numeric(summarise_draws(df, "median")[2,2]) - log(1)
  
  # Coverage - does the true value fall in the credible interval?
  lci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,2])
  uci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,3])
  
  cv_cumlogit_lr8[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  
  # MSE 
  mse_cumlogit_lr8[k] <- (as.numeric(summarise_draws(df, "median")[2,2]) - log(1))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_lr9[k] <- as.numeric(summarise_draws(df, "median")[2,2]) - log(1)
  
  # Coverage - does the true value fall in the credible interval?
  lci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,2])
  uci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,3])
  
  cv_cumlogit_lr9[k] <- ifelse(lci < log(1) & uci > log(1), 1, 0)
  
  # MSE 
  mse_cumlogit_lr9[k] <- (as.numeric(summarise_draws(df, "median")[2,2]) - log(1))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_lr10[k] <- as.numeric(summarise_draws(df, "median")[2,2]) - log(1.10)
  
  # Coverage - does the true value fall in the credible interval?
  lci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,2])
  uci <- as.numeric(summarise_draws(df, function(x) quantile(x, probs = c(0.025, 0.975)))[2,3])
  
  cv_cumlogit_lr10[k] <- ifelse(lci < log(1.10) & uci > log(1.10), 1, 0)
  
  # MSE 
  mse_cumlogit_lr10[k] <- (as.numeric(summarise_draws(df, "median")[2,2]) - log(1.10))^2
...
save(data, file = paste0("smalln_sym_11cat_diverg_slight",datnum,".Rdata"))
