...
setwd("/group/cebu1/BACKUP/Chris/Project_2/Main_Simulations/Large_n/Symmetric/Three_categories/Divergent/Moderate_effect")
...
load("largen_sym_3cat_diverg_modeff_dataset.Rdata")
attach(big_data)
...
for (k in s1[datnum]:s2[datnum]){
  ...
  pomod <- blrm(y~x1,pcontrast=list(c1=.(x1='0'), c2=.(x1='1'),
                  contrast=expression(c2-c1), sd=10000), conc = 1, seed=1234,iter=7500,chains=4,sampling.args = list(cores = 4))
  
  df <- as_draws_df(pomod$draws)
  
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_po1[k] <- as.numeric(summarise_draws(df, "median")[3,2]) - log(1)
  bias_cumlogit_po2[k] <- as.numeric(summarise_draws(df, "median")[3,2]) - log(1.50)
  ...
  # MSE 
  mse_cumlogit_po1[k] <- (as.numeric(summarise_draws(df, "median")[3,2]) - log(1))^2
  mse_cumlogit_po2[k] <- (as.numeric(summarise_draws(df, "median")[3,2]) - log(1.50))^2
  ...
  # Extract summary for proportional OR 
  # Bias for each cumulative logit 
  bias_cumlogit_ppo1[k] <- as.numeric(summarise_draws(df, "median")[3,2]) - log(1)
  ...
  # MSE 
  mse_cumlogit_ppo1[k] <- (as.numeric(summarise_draws(df, "median")[3,2]) - log(1))^2
...
save(data, file = paste0("largen_sym_3cat_diverg_modeff",datnum,".Rdata"))
