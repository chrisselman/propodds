...
```{r}
library(miceadds)
data <- do.call(rbind,lapply(paste0("largen_sym_11cat_randompo_modeff_slight",c(1:50),".Rdata"), load.Rdata2))
...
for(i in 1:nsim){
  ...
  MC_YMinusi_relbias7_ppo[i] = (exp(sum(bias_cumlogit_ppo7[-i])/(nsim-1) + log(1.5))-exp(log(1.5)))/(exp(log(1.5)))
  MC_YMinusi_relbias8_ppo[i] = (exp(sum(bias_cumlogit_ppo8[-i])/(nsim-1) + log(1.5))-exp(log(1.5)))/(exp(log(1.5)))
  MC_YMinusi_relbias9_ppo[i] = (exp(sum(bias_cumlogit_ppo9[-i])/(nsim-1) + log(1.5))-exp(log(1.5)))/(exp(log(1.5)))
  ...
  MC_YMinusi_relbias7_lcpo[i] = (exp(sum(bias_cumlogit_lcpo7[-i])/(nsim-1) + log(1.5))-exp(log(1.5)))/(exp(log(1.5)))
  MC_YMinusi_relbias8_lcpo[i] = (exp(sum(bias_cumlogit_lcpo8[-i])/(nsim-1) + log(1.5))-exp(log(1.5)))/(exp(log(1.5)))
  MC_YMinusi_relbias9_lcpo[i] = (exp(sum(bias_cumlogit_lcpo9[-i])/(nsim-1) + log(1.5))-exp(log(1.5)))/(exp(log(1.5)))
...
save(results, file = "vlargen_sym_11cat_randompo_modeff_slight_results.Rdata")
write.csv(results,file="G:/BACKUP/Chris/Project_2/Main_Simulations/Results/results_134.csv",row.names=F)
