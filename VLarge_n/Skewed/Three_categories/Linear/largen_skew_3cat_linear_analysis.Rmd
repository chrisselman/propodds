...
```{r}
library(miceadds)
data <- do.call(rbind,lapply(paste0("largen_skew_3cat_linear",1:50,".Rdata"), load.Rdata2))
...
effectsize <- rep("0.06 increase", 10)

truemod <- rep("Linear", 10)

controlprob <- rep("Skewed", 10)

sampsize <- rep(10000, 10)
...
save(results, file = "vlargen_skew_3cat_linear_results.Rdata")
write.csv(results,paste("G:/BACKUP/Chris/Project_2/Main_Simulations/Results/results_129.csv"),row.names=F)
